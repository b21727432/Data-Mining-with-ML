...
def build_model_minibatch(nn_hdim, N, num_passes=20000, print_loss=False):
...
modelminibatch = build_model_minibatch(3, 10, print_loss=True)

# Plot the decision boundary

plot_decision_boundary(lambda x: predict(modelminibatch, x))
plt.title("Decision Boundary for hidden layer size 3 with Minibatch Gradient")
...
Xtest3, ytest3 = make_blobs(n_samples=300, centers=3, n_features=2)
df = DataFrame(dict(x=Xtest3[:,0], y=Xtest3[:,1], label=ytest3))
